...
public class TableColumnsAutofitter {
	...
	public static int autoResizeTable(JTable aTable, boolean includeColumnHeaderWidth, int columnPadding, boolean fit) {
		...
		if (columnCount > 0) // must have columns !
		{
			...
			// tableHeader.setPreferredSize( headerDim );//commented out by
			// uhrand (caused resize bug)
			...
			if (fit && tableWidth != viewWidth) {
				int corr = (viewWidth -	tableWidth)	/ columnCount;
				tableWidth = 0;
				for (int i = 0; i < columnCount; i++) {
					columnWidth[i] += corr;
					tableWidth += columnWidth[i];
				...
				if (tableWidth != viewWidth) {
					columnWidth[columnCount - 1] +=	(viewWidth - tableWidth);
	...
	public static int autoResizeTable(JTable aTable, boolean includeColumnHeaderWidth, int columnPadding, boolean fit, int maxColumnWidth) {
		// NOTE: the "fit"-option has been added by uhrand
		int columnCount = aTable.getColumnCount();
		int currentTableWidth = aTable.getWidth();
		int tableWidth = 0;
		...
		if (columnCount > 0) // must have columns !
		{
			...
			for (int i = 0; i < columnCount; i++) {
				tableColumn = tableColumnModel.getColumn(i);
				// System.out.println(tableColumn.getHeaderValue()+" "+columnWidth[i]);
				tableColumn.setPreferredWidth(Math.min(columnWidth[i], maxColumnWidth)); //EDITED
			}
			aTable.invalidate();
			aTable.doLayout();
			aTable.repaint();
	...
	private TableColumnsAutofitter() {
	}
}